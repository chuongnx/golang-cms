...
func (CTRL *LoginController) Login() {
	...
	if VerifyUser(&user, username, password) {
		...
		if backTo != "" {
			CTRL.Redirect("/admin/"+backTo, 302)
		} else {
			CTRL.Redirect("/admin/profile/0/show", 302)
		}
	} else {
		CTRL.Redirect("/admin/register", 302)
	}
...
var AuthRequest = func(ctx *context.Context) {
	...
	if !ok && ctx.Input.URI() != "/admin/login" && ctx.Input.URI() != "/admin/register" {
